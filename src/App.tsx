...
import { HostelRequest, RequestCategory, RequestUrgency } from './types';
import AIAssistant from './components/AIAssistant';
import RoomHeatmap from './components/RoomHeatmap';
import StatsDashboard from './components/StatsDashboard';
import { Spotlight, CustomCursor, Magnetic, Tilt, RippleButton, CountUp, SkeletonShimmer, useReducedMotion, FloatingInput } from './components/InteractiveEffects';
...
// Helper component for floating atmospheric background elements
...
export default function App() {
  ...
  const [loginRole, setLoginRole] = useState<'student' | 'warden'>('student');
  ...
  const [selectedProfileId, setSelectedProfileId] = useState<string>('sarah');
  ...
  const [activeTab, setActiveTab] = useState<'student' | 'warden'>('student');
  ...
  // References to keep callbacks current and robust against react state closures
  const requestsRef = useRef<HostelRequest[]>([]);
  const selectedRequestRef = useRef<HostelRequest | null>(null);
  ...
  // Simulated AI Analyzer Stages State
  ...
  // Warden Filter and Action States
  const [wardenFilter, setWardenFilter] = useState<'all' | 'pending' | 'maintenance' | 'mess' | 'permission' | 'urgent'>('all');
  ...
  // Real-time snapshot listener for Firestore requests
  useEffect(() => {
    if (!isLoggedIn) return;

    const q = query(collection(db, 'requests'), orderBy('createdAt', 'desc'));
    const unsubscribeSnapshot = onSnapshot(q, async (snapshot) => {
      let data: HostelRequest[] = [];
      snapshot.forEach((docSnap) => {
        data.push({ id: docSnap.id, ...docSnap.data() } as HostelRequest);
      });

      // Seeding initial requests if Firestore is empty
      if (data.length === 0) {
        const seedData = [
          {
      ...
      if (requestsRef.current.length > 0) {
        ...
        if (data.length > requestsRef.current.length) {
          showToast(`🔔 REAL-TIME: New student submission in active ledger.`, 'info');
        }
      }
  ...
  // Telemetry Ping Fluctuation Effect
  useEffect(() => {
  ...
  // Handle Q&A send message to Aura chat — passes full history for Gemini context
  const handleSendToAura = async (message: string, history: any[] = []): Promise<string> => {
  ...
  // Submit new student request with a premium simulated multi-stage AI extraction
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    // Multi-stage simulated delay for beautiful visual feedback
    const advanceStage = (stageNum: number, ms: number) => {
    ...
    try {
      ...
      // Write directly to Firestore cloud database in real-time!
  ...
  // Handle Warden Action (Approve, Reject, Resolve, Assign)
  const handleWardenAction = async (action: 'approve' | 'reject' | 'resolve' | 'assign') => {
    if (!selectedRequest) return;
    setIsActioning(true);
    try {
      let nextStatus: HostelRequest['status'] = selectedRequest.status;
      ...
      // Direct Firestore document update in real-time!
  ...
  // Quick fill samples for testing
  const quickFill = (type: 'electrical' | 'gatepass' | 'water') => {
    if (type === 'electrical') {
  ...
  // Filter requests for warden view
  ...
  const getCategoryIcon = (cat: RequestCategory) => {
    switch (cat) {
      case 'maintenance': return <Wrench className="w-4 h-4 text-amber-500 animate-pulse" />;
      case 'mess': return <Utensils className="w-4 h-4 text-emerald-500" />;
      case 'permission': return <Key className="w-4 h-4 text-indigo-500" />;
      case 'complaint': return <AlertOctagon className="w-4 h-4 text-rose-500 animate-bounce" />;
  ...
  const getStatusBadge = (status: HostelRequest['status']) => {
    switch (status) {
      ...
      case 'ai_evaluated':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200/50 shadow-2xs animate-pulse">
  ...
  // Render Urgency Badge helper
  const getUrgencyBadge = (urgency: RequestUrgency) => {
    switch (urgency) {
      case 'critical':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold bg-rose-900 text-white animate-pulse shadow-sm">
            CRITICAL
          </span>
        );
      case 'high':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-rose-100 text-rose-800 border border-rose-200">
            HIGH
          </span>
        );
      case 'medium':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-200">
            MEDIUM
          </span>
        );
      case 'low':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-semibold bg-slate-100 text-slate-600 border border-slate-200">
            LOW
          </span>
        );
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-900 flex flex-col font-sans selection:bg-indigo-500/20 selection:text-indigo-900 relative overflow-x-hidden" id="app-container">
        ...
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              ...
              className={`p-4 rounded-xl border pointer-events-auto shadow-md flex items-center gap-3 backdrop-blur-md ${
                toast.type === 'success' 
      ...
      {!isLoggedIn ? (
        <div className="flex w-full min-h-screen bg-slate-50 relative overflow-hidden" id="split-screen-login">
          ...
          <div className="hidden lg:flex flex-1 relative flex-col justify-center p-16 z-10 overflow-hidden select-none">
            ...
            {/* ── Aurora layers ────────────────────────────────── */}
            <div className="aurora-1 absolute top-[-20%] left-[-15%] w-[70%] h-[70%] rounded-full opacity-50 blur-3xl"
              style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.55) 0%, rgba(139,92,246,0.3) 50%, transparent 80%)' }} />
            <div className="aurora-2 absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full opacity-40 blur-3xl"
              style={{ background: 'radial-gradient(circle, rgba(14,165,233,0.5) 0%, rgba(56,189,248,0.25) 50%, transparent 80%)' }} />
            <div className="absolute top-[40%] right-[5%] w-[40%] h-[40%] rounded-full opacity-25 blur-3xl"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      {['Maintenance','Permission','Mess'].map((tag, i) => (
            ...
            >
              ...
              <div className="mb-10">
                ...
                <p className="text-[0.9375rem] text-slate-500 mt-2 font-medium leading-relaxed">
                  {isSignUp
                    ? 'Join the secure residential management ledger.'
                    : 'Sign in to manage operations and track requests.'}
                </p>
              </div>
              ...
              <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/70 mb-8 gap-1.5" id="login-role-selector">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2.5 py-3.5 rounded-xl text-xs font-bold transition-all duration-300 cursor-none ${
                    loginRole === 'student'
                      ? 'bg-white text-indigo-700 shadow-md border border-slate-200/80 scale-[1.02]'
              ...
              <div className="mt-7">
                <RippleButton
                  disabled={
                    ...
                    !loginEmail.trim() ||
                    !loginPassword.trim() ||
                    (isSignUp && loginRole === 'student' && (!loginStudentName.trim() || !loginRoomNumber.trim()))
                  }
                  onClick={async () => {
                    if (!loginEmail.includes('@') || !loginEmail.includes('.')) {
                      showToast('Please enter a valid email address.', 'warning'); return;
                    }
                    // Warden passcode check — only on sign-up to gate access
                    if (isSignUp && loginRole === 'warden' && loginPassword !== 'admin' && loginPassword !== 'warden123') {
                      showToast('Incorrect Warden passcode. Use the authorized passcode to register.', 'warning'); return;
                    }
                    // Password length check — skip for valid warden passcodes (e.g. 'admin' is 5 chars)
                    ...
                    setIsLoggingIn(true);
                    setLoginProgress(10);
                    const updateProgress = (amt, ms) => new Promise(resolve => setTimeout(() => { setLoginProgress(amt); resolve(undefined); }, ms));
                    try {
          ...
          <div className="flex w-full min-h-screen bg-slate-50 overflow-hidden" id="premium-dashboard">
            {/* Sidebar Navigation */}
                ...
                <div className="space-y-1 mb-8">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all ${
                      activeTab === 'student' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 border border-transparent'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all ${
                      activeTab === 'warden' ? 'bg-sky-500/10 text-sky-400 border border-sky-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 border border-transparent'
                    }`}
                  >
            ...
            <main className="flex-1 flex flex-col relative h-screen overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-8 md:p-12 z-10 relative custom-scrollbar">
                ...
                {error && (
                  <div className="mb-8 p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-center justify-between gap-3 text-rose-900 text-sm shadow-md">
                            ...
                            <form onSubmit={handleSubmitRequest} className="space-y-6">
                              <div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                  {(['maintenance', 'permission', 'mess', 'complaint'] as RequestCategory[]).map((cat) => (
                                    <button
                                      key={cat}
                                      type="button"
                                      onClick={() => setCategory(cat)}
                                      className={`py-4 px-2 rounded-2xl text-xs font-bold capitalize transition-all flex flex-col items-center justify-center gap-2 border ${
                                        category === cat
                                ...
                                {isSubmitting && (
                                  ...
                                  >
                                    ...
                                    <p className="text-xs font-mono text-white/90 bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                                      ...
                                      {aiAnalysisStage === 2 && "🧠 Extracting hazard parameters and timelines..."}
                                      {aiAnalysisStage === 3 && "📂 Evaluating risk severity profiles..."}
                                      {aiAnalysisStage === 4 && "🚀 Computing optimal routing matrix..."}
                                      {aiAnalysisStage === 5 && "🎉 Ticket packet synthesized and dispatched."}
                                    </p>
                                  </motion.div>
                                )}
                              </AnimatePresence>
                        ...
                        <div className="lg:col-span-5 flex flex-col h-[750px]">
                          <div className="glass-card rounded-[2rem] p-6 border border-white shadow-lg flex flex-col h-full overflow-hidden relative">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2 z-10 relative">
                              {isLoading ? (
                                Array(3).fill(0).map((_, i) => <SkeletonShimmer key={i} className="h-32 w-full rounded-2xl" />)
                              ) : requests.length > 0 ? (
                                requests.map((req) => (
                                  <motion.div
                                    key={req.id}
                            ...
                            <div className="w-1/2 flex flex-col border-r border-slate-100 pr-6">
                              <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="relative">
                                  ...
                                  <select
                                    value={wardenFilter}
                                    onChange={(e) => setWardenFilter(e.target.value as any)}
                                  ...
                                  <div
                                    ...
                                    className={`p-4 rounded-2xl border transition-all cursor-pointer ${
                                      selectedRequest?.id === req.id
                                        ? 'bg-slate-900 border-slate-800 text-white shadow-lg scale-[1.02]'
                              ...
                              {selectedRequest ? (
                                ...
                                >
                                  ...
                                  {selectedRequest.aiAssessment && (
                                    <div className="p-5 bg-gradient-to-br from-indigo-50 to-sky-50 rounded-2xl border border-indigo-100 shadow-sm relative overflow-hidden">
                                  ...
                                  <div className="pt-4 border-t border-slate-100 space-y-4">
                                    ...
                                    {selectedRequest.category === 'permission' ? (
