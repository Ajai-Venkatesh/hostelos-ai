...
import { Send, Sparkles, Utensils, Key, Calendar, Phone, Wifi, Trash2, Bot } from 'lucide-react';
...
interface AIAssistantProps {
  ...
  userId?: string; // optional: scoped chat per user
}
...
function renderMarkdown(text: string) {
  return text.split('\n').map((line, i) => {
    ...
    if (line.trim().startsWith('* ') || line.trim().startsWith('- ')) {
      ...
      return (
        <li key={i} className="ml-4 list-disc text-[0.8375rem] text-slate-700 mt-0.5 leading-relaxed">
          {parseBold(rest)}
        </li>
      );
    }
    ...
    // Normal paragraph
    return (
      <p key={i} className="text-[0.8375rem] text-slate-700 leading-relaxed mt-1 min-h-[0.5rem]">
        {parseBold(line)}
      </p>
    );
  });
...
// ── Streaming message bubble (last AI message gets typewriter) ───────────────
function AuraBubble({ text, isLatest }: { text: string; isLatest: boolean }) {
  ...
  return (
    <div className="space-y-0.5">
      ...
      {isLatest && displayed.length < text.length && (
        <span className="inline-block w-0.5 h-3.5 bg-indigo-500 ml-0.5 animate-pulse rounded-full align-middle" />
      )}
    </div>
  );
}
...
// ── Main component ────────────────────────────────────────────────────────────
export default function AIAssistant({ onSendMessage, userId = 'global' }: AIAssistantProps) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const msgs: ChatMessage[] = snap.docs.map(d => ({
        ...
        role: d.data().role,
        text: d.data().text,
        timestamp: d.data().timestamp || new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      }));

      // If empty, add welcome message locally (don't write to Firestore)
      if (msgs.length === 0) {
        setMessages([{
          id: 'welcome',
          role: 'model',
          text: "👋 Hi! I'm **Aura**, your AI Resident Assistant powered by Gemini.\n\nI know everything about hostel timings, gate rules, laundry, contacts, and Wi-Fi. Ask me anything!",
      ...
      setFirestoreReady(true);
      // Fallback: no Firestore, use local state
      setMessages([{
        id: 'welcome',
        role: 'model',
        text: "👋 Hi! I'm **Aura**, your AI Resident Assistant powered by Gemini.\n\nI know everything about hostel timings, gate rules, laundry, contacts, and Wi-Fi. Ask me anything!",
  ...
  const handleSend = useCallback(async (textToSend: string) => {
    ...
    try {
      // Pass full history (excluding welcome) for context
      ...
      // Track which message gets the typewriter effect
      if (aiDocRef) setLatestAIId(aiDocRef.id);
      else {
        // Firestore failed — update local state directly
      ...
      const errText = err?.message?.includes('temporarily')
        ? err.message
  ...
  return (
    <div className="flex flex-col h-[640px] bg-white/75 backdrop-blur-3xl rounded-[2rem] overflow-hidden shadow-2xl border border-white/50" id="ai-assistant-card">
                ...
                <div
                  className={`px-4 py-3 rounded-2xl shadow-sm max-w-full ${
                    msg.role === 'user'
                      ...
                      : 'bg-white border border-slate-100/80 text-slate-800 rounded-bl-sm'
                  }`}
                >
                  {msg.role === 'user' ? (
                    <p className="text-[0.8375rem] leading-relaxed whitespace-pre-wrap">{msg.text}</p>
                  ) : (
                    <AuraBubble
                      text={msg.text}
