...
import { Building, ShieldAlert, Sparkles, User, AlertCircle, Wrench, Utensils, Key, FileText } from 'lucide-react';
import { Tilt } from './InteractiveEffects';

interface RoomHeatmapProps {
  requests: HostelRequest[];
  onSelectRequest?: (req: HostelRequest) => void;
  onRoomSelect?: (roomNum: string) => void;
  currentRoom?: string;
}

export default function RoomHeatmap({ requests, onSelectRequest, onRoomSelect, currentRoom }: RoomHeatmapProps) {
  const [activeWing, setActiveWing] = useState<'A' | 'B'>('B');
  const [hoveredRoom, setHoveredRoom] = useState<string | null>(null);

  // Generate rooms for floors 1 to 3, rooms 1 to 4
  const floors = [3, 2, 1];
  const roomIndices = [1, 2, 3, 4];

  // Map of category to visual colors
  const getCategoryColor = (cat: string) => {
    switch (cat) {
      case 'maintenance': return 'bg-amber-500 border-amber-600 shadow-amber-400/40 text-amber-950';
      case 'permission': return 'bg-indigo-500 border-indigo-600 shadow-indigo-400/40 text-indigo-950';
      case 'mess': return 'bg-emerald-500 border-emerald-600 shadow-emerald-400/40 text-emerald-950';
      default: return 'bg-rose-500 border-rose-600 shadow-rose-400/40 text-rose-950';
    }
  };

  const getCategoryBgLight = (cat: string) => {
    switch (cat) {
      case 'maintenance': return 'bg-amber-50 text-amber-800 border-amber-100   ';
      case 'permission': return 'bg-indigo-50 text-indigo-800 border-indigo-100   ';
      case 'mess': return 'bg-emerald-50 text-emerald-800 border-emerald-100   ';
      default: return 'bg-rose-50 text-rose-800 border-rose-100   ';
    }
  };
  ...
  const getCategoryIcon = (cat: string) => {
    switch (cat) {
      ...
      case 'permission': return <Key className="w-3.5 h-3.5 text-indigo-600 " />;
      case 'mess': return <Utensils className="w-3.5 h-3.5 text-emerald-600 " />;
      default: return <FileText className="w-3.5 h-3.5 text-rose-600 " />;
    }
  };
            ...
            <div key={floor} className="flex items-center gap-3">
              ...
              <div className="grid grid-cols-4 gap-2 flex-1">
                {roomIndices.map((idx) => {
                  ...
                  // Find requests associated with this room
                  ...
                  return (
                    ...
                    >
                      <motion.button
                        ...
                        className={`w-full py-3.5 px-2 rounded-xl border text-center transition-all cursor-pointer relative flex flex-col items-center justify-center gap-1 shadow-sm ${
                          isSelected 
                            ? 'border-indigo-400 bg-indigo-50/80 text-indigo-900 ring-4 ring-indigo-500/20'
                            : activeIssue
                              ? 'border-rose-300 bg-rose-50/80 text-rose-900 hover:bg-rose-100/50'
                        ...
                        {activeIssue ? (
                          <div className="relative flex h-2 w-2 mt-0.5">
                            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                              activeIssue.category === 'maintenance' ? 'bg-amber-400' : 'bg-rose-400'
                            }`}></span>
                            <span className={`relative inline-flex rounded-full h-2 w-2 ${
                              activeIssue.category === 'maintenance' ? 'bg-amber-500' : 'bg-rose-500'
                            }`}></span>
                          </div>
                        ) : (
                        ...
                        {hoveredRoom === roomNum && (
                          ...
                          >
                            ...
                            {activeIssue ? (
                              ...
                              <p className="text-[10px] text-slate-400 italic">No active maintenance issues or pending gate passes recorded.</p>
                            )}
                          </motion.div>
                        )}
                      </AnimatePresence>
