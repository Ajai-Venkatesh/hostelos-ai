...
export default function CinematicIntro({ onComplete }: CinematicIntroProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        onComplete();
      }, 1000); // 1s for the final dissolve to finish
    }, 6500);

    return () => clearTimeout(timer);
  }, []); // Empty dependency array prevents timer cancellation on re-renders

  // Particle System using Vanilla JS and Canvas (No React Re-renders)
  useEffect(() => {
    ...
    // Scene state tracking for particles based on time
    ...
    const render = () => {
      ...
      // Phase 1: Ambient particles (0-2s)
      // Phase 2: Flow toward center (2-3.5s)
      // Phase 3: Orbiting ring (3.5-5s)
      // Phase 4: Dissolve explosion (6.5s+)
      ...
      particles.forEach((p, i) => {
        if (elapsed < 2000) {
          ...
          if (p.y < 0) p.y = height;
          if (p.y > height) p.y = 0;
          ctx.fillStyle = `rgba(125, 211, 252, ${p.opacity * (elapsed / 1000)})`;
        } 
          ...
          // Orbit
          const radius = 60 + Math.sin(p.phase + elapsed * 0.002) * 10;
          const angle = p.phase + elapsed * 0.003;
          ...
          // Explosion / Dissolve
          const explosionProgress = (elapsed - 6500) / 1000;
          const angle = p.phase;
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[9999] bg-[#030712] overflow-hidden intro-container pointer-events-none w-full h-[100vh] flex justify-center items-center">
